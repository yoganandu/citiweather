...
data class Main(val temp: Double, val pressure: Int,
                val humidity: Int, val temp_min: Double,
                val temp_max: Double, val sea_level: Double,
...
data class Sys(val type: Int, val id: Int,
               val message: Double, val country: String,
               val sunrise: Long, val sunset: Long)

data class WeatherInfo(val coord: Coordinates, val weather: List<Weather>,
                       val base: String, val main: Main,
                       val wind: Wind, val clouds: Clouds,
                       val rain: Rain, val snow: Snow,
                       @SerializedName("dt") val date: Long, val sys: Sys,
                       @SerializedName("id") val cityId: Int, @SerializedName("name") val cityName: String,
                       @SerializedName("cod") val responseCode: Int)
data class WeatherAPIResponse(val count: Int, @SerializedName("list") val weatherList: List<WeatherInfo>)
data class WeatherAPIError(@SerializedName("cod") val errorCode: Int, @SerializedName("message") val errorMessage: String)
